...
angular.module('overallPercentage.controllers', [])
	.controller('overallPercentageCtrl', ['$scope', '$http', 'ReportRestClient', 'ReportService', function($scope, $http, restClient, reportService) {
		$scope.charts = [
    	...
    	$scope.selectReportDenominotor = function() {
    		$scope.currentDenominator = $scope.selectedDenominatorOption.id;
    		$scope.showReport();
    	};
    	...
    	$scope.showReport = function()
        {
        	...
        	if ( $scope.selectedTime == 0 ) {
        		params = {year: $scope.currentYear, denominator: $scope.currentDenominator};
        		reportService.setMonthOfYear(-1);
        	}
        	if ( $scope.selectedTime == 1 ) {
        		params = {year: $scope.currentYear, monthOfYear: $scope.currentMonth, denominator: $scope.currentDenominator};
