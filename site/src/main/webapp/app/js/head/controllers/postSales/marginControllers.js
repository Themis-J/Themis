...
angular.module('postSalesMargin.controllers', [])
	.controller('postSalesMarginCtrl', ['$scope', 'ReportRestClient', 'ReportService', 'config', function($scope, restClient, reportService, config) {
		/**
		 * Global functions
		 */
        ...
        $scope.draw = function (restClient, params) {
            ...
            restClient(params, function(data) {
		        ...
		        jQuery("#report_list").jqGrid({
						...
						datatype: "local",
					   	colNames:['部门', '经销商代码', '名称', '部门售后毛利', '经销商售后毛利'],
					   	colModel:[
					   	    {name:'departmentName',index:'departmentName', width:55},
					   		{name:'id',index:'id', width:55},
					   		{name:'name',index:'name', sorttype: function (cellValues, rowData) {return rowData.totalAmount;}, width:100},
