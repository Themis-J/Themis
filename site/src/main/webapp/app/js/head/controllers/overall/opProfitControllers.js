...
angular.module('overallOpProfit.controllers', [])
    .controller('overallOpProfitCtrl', ['$scope', 'ReportRestClient', 'ReportService', 'config', function ($scope, restClient, reportService, config) {
        /**
         * Global functions
         */
        ...
        $scope.draw = function (restClient, params) {
            ...
            restClient(params, function (data) {
                ...
                jQuery("#report_list").jqGrid({
                    ...
                    datatype: "local",
                    colNames: ['部门', '经销商代码', '名称', '部门总毛利', '总运营利润'],
                    colModel: [
                        {name: 'departmentName', index: 'departmentName', width: 55},
                        {name: 'id', index: 'id', width: 55},
                        {name: 'name', index: 'name', sorttype: function (cellValus, rowData) {
                            return (rowData.totalAmount + rowData.id).lpad('0', 100);
                        }, width: 100},
