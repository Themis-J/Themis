...
angular.module('keyAccountReceivablePercentage.controllers', [])
	.controller('keyAccountReceivablePercentageCtrl', ['$scope', 'ReportRestClient', 'ReportService', 'config', function($scope, restClient, reportService, config) {
		/**
		 * Global functions
		 */
    	...
    	$scope.showReport = function() {
        	var params = {year: reportService.getCurrentYear(), monthOfYear: reportService.getMonthOfYear()};
        	$scope.draw(restClient(config.currentMode).queryAccountReceivablePercentageReport, params);
        };
        ...
        $scope.draw = function (restClient, params) {
	            ...
	            restClient(params, function(data) {
	            	...
	            	for (var i=0; i<previousDetail.length; i++) {
	            		...
	            		chartData.series.previous[i] = previousDetail[i].amount.amount * 100;
	            		chartData.series.previousReference[i] = previousDetail[i].amount.reference * 100;
	            		...
	            		chartData.gridData[i].amount = previousDetail[i].amount.amount * 100;
	            	};
