    ...
    .controller('renyuanCtrl', ['$scope', 'Dealer', 'DealerService', '$q', '$timeout',
        function ($scope, Dealer, DealerService, $q, $timeout) {
            ...
            if (DealerService.getSelectedDept() == 11) {
                ...
                $scope.feeDefer = $q.defer();
                $scope.summaryDefer = $q.defer();

                $q.all([$scope.feeDefer.promise, $scope.summaryDefer.promise])
            ...
            $scope.autoSaveFee = function (employeeFee) {
                if (!this.form.$invalid) {
                    ...
                    for (var i = 0; i < itemsToSave.length; i++) {
                        ...
                        postData.detail.push({
                            ...
                            amount: employeeFee.data[itemsToSave[i]].amount,
                            feeTypeID: employeeFee.data[itemsToSave[i]].feeTypeID
                        });

                        var success = function () {
                            employeeFee.sign = "icon-check-sign green";
                        ...
                        var failed = function () {
                            employeeFee.sign = "icon-remove-sign red";
            ...
            $scope.autoSaveFeeSummary = function (employeeSummary) {
                if (!this.form.$invalid) {
                    var postData = {};
                    postData.dealerID = DealerService.getDealerId();
                    ...
                    postData.detail.push({
                        ...
                        amount: employeeSummary.amount
                    });

                    var success = function () {
                        employeeSummary.sign = "icon-check-sign green";
                        var currentDate = new Date();
                        $scope.autoSaveTime = "上次自动保存于" + currentDate.getHours() + "点" + currentDate.getMinutes() + "分" + currentDate.getSeconds() + "秒";
                        $scope.autoSaveClass = "text-success";
                        $scope.markEdit();
                    ...
                    var failed = function () {
                        employeeSummary.sign = "icon-remove-sign red";
                        $scope.autoSaveTime = "自动保存失败";
                        $scope.autoSaveClass = "text-error";
                    };
